...
public class MLSystemManager {
	 ...
	 */
	public SupervisedLearner getLearner(String model, Random rand, double numHiddenLayers, 
			String hiddenNodes, double learningRate, double momentum) throws Exception
	{
		if (model.equals("baseline")) return new BaselineLearner();
		// else if (model.equals("perceptron")) return new Perceptron(rand);
		else if (model.equals("neuralnet")) { return new NeuralNet(rand, (int) numHiddenLayers, 
				hiddenNodes, learningRate, momentum); }
	...
	public void run(String[] args) throws Exception {
		...
		double numHiddenLayers = parser.getHiddenLayers();
		String hiddenNodes = parser.getHiddenNodes();
		double learningRate = parser.getLearningRate();

		// Load the model
		SupervisedLearner learner = getLearner(learnerName, rand, numHiddenLayers, 
				hiddenNodes, learningRate, momentum);
	...
	private class ArgParser {
		...
		boolean verbose;
		boolean normalize;
		//added for neural net
		double momentum = 0.0;
		double learningRate = 0.0;
		double hiddenLayerSize = 0;
		String hiddenNodes = "";
		...
		public ArgParser(String[] argv) {
			try{
			 	...
			 	for (int i = 0; i < argv.length; i++) {
			 		...
			 		{
			 			momentum = Double.valueOf(argv[++i]);
			 		}
			 		else if (argv[i].equals("-LR")) 
			 		{
			 			learningRate = Double.valueOf(argv[++i]);
			 		}
			 		else if (argv[i].equals("-HLS")) 
			 		{
			 			hiddenLayerSize = Double.valueOf(argv[++i]);
			 		}
			 		else if (argv[i].equals("-HN")) 
			 		{
			 			hiddenNodes = argv[++i];
			 		}
				...
				if (arff == null || learner == null || evaluation == null)
				{
					...
					System.out.println("OPTIONS:");
					System.out.println("-V Print the confusion matrix and learner accuracy on individual class values");
					System.out.println("-M @value Only used for neural net. Sets momentum term in neural net to @value");
		...
		public double getHiddenLayers() { return hiddenLayerSize; }
		public double getLearningRate() { return learningRate; }
		public String getHiddenNodes() { return hiddenNodes; }
	}
