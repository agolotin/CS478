...
//		oldOut.println(os.toString());
	...
	private void backward(double target) throws Exception { 
		...
		ArrayList<ArrayList<ArrayList<Double>>> tempDeltaWeights = new ArrayList<ArrayList<ArrayList<Double>>>();
		for (int i = 0; i < numLayers - 1; i++) { // nothing to change on the output layer
			//input layer to hidden layer
			if (i == 0) {
				tempDeltaWeights.add(deltaWeightUpdate(this.inputs, deltaError.get(i), i));
			}
			// hidden to hidden
			else {
				tempDeltaWeights.add(deltaWeightUpdate(innerHidden1, deltaError.get(i), i));
		...
		for (int i = 0; i < numLayers - 1; i++) { // nothing to change on the output layer
			//input layer to hidden layer
			if (i == 0) 
				changeDelta(this.inputs, tempDeltaWeights.get(i));
			else 
				changeDelta(hidden.next(), tempDeltaWeights.get(i));
		}
		this.deltaWeigths = tempDeltaWeights; // save the delta weights from previous layer
	}
	 ...
	 */
	private <T extends Node> ArrayList<ArrayList<Double>> deltaWeightUpdate(ArrayList<T> currentLayer, 
			ArrayList<Double> nextLayerError, int outerLayer) {
		...
		for (int i = 0; i < currentLayer.size(); i++) {
			...
			for (int k = 0; k < singleNode.getVectorSizeToNextLayer(); k++) {
				double deltaChange = momentumDeltaRule(nextLayerError.get(k), 
							singleNode.getActivationValue(), this.deltaWeigths.get(outerLayer).get(i).get(k));
